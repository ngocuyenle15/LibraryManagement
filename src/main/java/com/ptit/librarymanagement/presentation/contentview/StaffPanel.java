...
//        this.bookService = TransactionManager.createService(BookService.class, DbConnection.getConnection());
    ...
    private void initComponent() {
        this.scrollTable = new ScrollTable(new String[]{"Id", "Họ và tên", "Giới tính", "Ngày sinh", "Địa chỉ", "Lương", "Số điện thoại", "Email", "Username", "Role"});
        ...
        this.scrollTable.getColumnModel().getColumn(4).setCellRenderer(leftRenderer);
        this.scrollTable.getColumnModel().getColumn(5).setCellRenderer(leftRenderer);
        this.scrollTable.getColumnModel().getColumn(6).setCellRenderer(centerRenderer);
        this.scrollTable.getColumnModel().getColumn(7).setCellRenderer(leftRenderer);
        this.scrollTable.getColumnModel().getColumn(8).setCellRenderer(leftRenderer);
        this.scrollTable.getColumnModel().getColumn(9).setCellRenderer(centerRenderer);
        ...
        btn.put("changePassword", new ButtonToolBar("Đổi mật khẩu", "icon/resetpass.svg"));
        this.functionToolBar = new FunctionToolBar(btn);
        this.integratedSearch = new IntegratedSearch(new String[]{"Id", "Họ và tên", "Username"});
