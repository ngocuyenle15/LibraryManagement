...
public class PublisherPanel extends JPanel {
    ...
    private void initComponent () {
        ...
        integratedSearch = new IntegratedSearch(new String[]{"Id", "Tên nhà xuất bản"});
        ...
        scrollTable = new ScrollTable(new String[]{"Id", "Tên nhà xuất bản", "Địa chỉ", "Số điện thoại", "Email"});
        scrollTablePublisher = new JScrollPane();

        scrollTablePublisher.setViewportView(scrollTable);
...
//        columnModel.getColumn(0).setPreferredWidth(2);
        ...
        main.add(scrollTablePublisher);

        bookInPublisher = new JPanel();
        bookInPublisher.setBackground(backgroundColor);
        bookInPublisher.setLayout(new FlowLayout(0, 4, 10));
        bookInPublisher.setPreferredSize(new Dimension(400, 800));

        scrollPane = new JScrollPane(bookInPublisher, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
    ...
    public void loadDataInTable() {
        ...
        for (PublisherDTO publisherDTO : model.getPublisherDTOS()) {
            tableModel.addRow(publisherDTO.getRow());
        }
    }

    public void showListBookOfPublisher() {
        bookInPublisher.removeAll();
        JLabel tit = new JLabel("Sách thuộc nhà xuất bản");
        tit.setFont(new Font(FlatRobotoFont.FAMILY, 1, 16));
        bookInPublisher.add(tit);
        ...
        for (int i = 0; i < size; i++) {
            ...
            bookInPublisher.add(listItem[i]);
        }
        bookInPublisher.repaint();
